...
function renderCategories(){
  ...
  qs('#category-filter').insertAdjacentHTML('beforeend', STORE.categories.map((cat)=>`<option value="${escapeHtml(cat.id)}">${escapeHtml(cat.name)}</option>`).join(''));
}
...
function renderProducts(){
  ...
  productGrid.innerHTML = filtered.length ? filtered.map((product) => `
    <article class="product-card">
      <div class="product-art" aria-hidden="true"><div class="shape"></div></div>
...
function renderSelected(){
  ...
  qs('#message').value = items.length ? `Интересувам се от:\n${names}\n\nМоля за цена, наличност и подходяща конфигурация.` : '';
}
